        ...
        <div className="p-4 space-y-2">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1">
              ...
              <span className="text-xs text-amber-600 font-semibold">{rating}</span>
            </div>
            {!course.isFree && (
...
function EditProfileModal({
  ...
  const inputCls = 'w-full bg-white border border-gray-300 rounded-xl px-3 py-2.5 text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:border-indigo-500 focus-visible:ring-2 focus-visible:ring-indigo-500 transition-colors'
  const readonlyCls = 'w-full bg-slate-50 border border-gray-200 rounded-xl px-3 py-2.5 text-sm text-gray-400 focus:outline-none transition-colors cursor-not-allowed'
    ...
    >
      ...
      >
        <div className="flex items-center justify-between p-5 border-b border-gray-100">
          <h2 className="text-lg font-bold text-gray-900">Profili düzənlə</h2>
        ...
        <div className="p-5 space-y-4">
          <div>
            <label className="block text-xs text-gray-500 mb-1.5">Qısa bio</label>
            <input
              value={form.bio}
          ...
          <div>
            <label className="block text-xs text-gray-500 mb-1.5">Haqqında (ətraflı)</label>
            <textarea
              value={form.longBio}
              readOnly
              rows={4}
            ...
            />
            <p className="mt-1 text-[11px] text-gray-400">{unsupportedFieldHelper}</p>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs text-gray-500 mb-1.5">Şəhər</label>
              <input
                value={form.city}
                readOnly
            ...
            <div>
              <label className="block text-xs text-gray-500 mb-1.5">Təcrübə (il)</label>
              <input
                type="number"
          ...
          <div>
            <label className="block text-xs text-gray-500 mb-1.5">Fənn</label>
            <input
              value={form.subject}
          ...
          <div>
            <label className="block text-xs text-gray-500 mb-1.5">Məktəb / Universitet</label>
            <input
              value={form.school}
              readOnly
              className={readonlyCls}
        ...
        <div className="flex gap-3 p-5 border-t border-gray-100">
          <button
            onClick={onClose}
            className="flex-1 py-2.5 rounded-xl border border-gray-300 bg-white text-gray-600 hover:text-gray-900 hover:bg-gray-50 transition-colors text-sm"
          >
            Ləğv et
          </button>
          <button
    ...
    return (
      <div className="min-h-screen bg-slate-50 animate-pulse">
        ...
        <div className="max-w-5xl mx-auto px-4 -mt-16 space-y-4">
          ...
          <div className="h-4 bg-slate-200 rounded-xl w-80" />
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen bg-slate-50 text-gray-900 flex items-center justify-center px-4">
        <div className="max-w-md w-full text-center space-y-5">
          <div className="w-14 h-14 mx-auto rounded-2xl bg-rose-50 border border-rose-200 flex items-center justify-center text-rose-500">
            !
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <button
              onClick={() => refetch()}
              disabled={isFetching}
              className="px-4 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-sm transition-colors disabled:opacity-50"
            >
              Yenidən yoxla
            </button>
            <Link
              to="/courses"
              className="px-4 py-2.5 rounded-xl border border-gray-300 bg-white text-gray-700 hover:text-gray-900 hover:bg-gray-50 transition-colors text-sm"
            >
              Kurslara qayıt
            </Link>
          </div>
    ...
    return (
      <div className="min-h-screen bg-slate-50 text-gray-900 flex items-center justify-center px-4">
        <div className="max-w-md w-full text-center space-y-3">
          <div className="w-14 h-14 mx-auto rounded-2xl bg-white border border-gray-200 flex items-center justify-center text-gray-400">
            ?
          </div>
    ...
    <div className="min-h-screen bg-slate-50 text-gray-900">
      ...
      <div className="relative h-72 overflow-hidden">
        ...
        <button
          ...
          className="absolute top-4 left-4 flex items-center gap-2 px-3 py-1.5 bg-white/90 backdrop-blur-sm border border-white/60 rounded-lg text-xs font-semibold text-gray-700 hover:text-gray-900 hover:bg-white transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500"
        >
        ...
        {isOwner && (
          <div className="absolute top-4 right-4">
            <button className="flex items-center gap-2 px-3 py-1.5 bg-white/90 backdrop-blur-sm border border-white/60 rounded-lg text-xs font-semibold text-gray-700 hover:text-gray-900 hover:bg-white transition-colors">
      ...
      <div className="max-w-5xl mx-auto px-4">
        <div className="flex flex-col sm:flex-row sm:items-end gap-4 -mt-16 relative z-10 pb-6 border-b border-gray-200">
          {/* Avatar — negative margin pulls it over cover */}
          <div className="relative shrink-0">
            <div className="relative w-28 h-28 sm:w-32 sm:h-32 rounded-2xl bg-gradient-to-br from-indigo-600 to-purple-700 border-4 border-white flex items-center justify-center text-4xl font-bold text-white shadow-xl">
              {teacher.avatar ? (
          ...
          <div className="flex-1 pt-2">
            ...
            <p className="text-gray-500 text-sm mb-2">{teacherSubject} · {teacherCity}</p>
            <p className="text-gray-600 text-sm max-w-xl">{teacherBio}</p>
              ...
              <div className="flex gap-2 mt-3">
                {socialLinks.map(link => (
                  <a
                    ...
                    className="w-8 h-8 flex items-center justify-center rounded-lg bg-white hover:bg-gray-50 border border-gray-200 transition-colors"
          ...
          {isOwner && (
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-sm font-semibold transition-colors shrink-0"
            >
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                <path d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
              </svg>
        ...
        {(introVideoUrl || isOwner) && (
          <div className="py-6 border-b border-gray-200">
            ...
            {introVideoUrl ? (
              <div className="aspect-video max-w-2xl rounded-2xl overflow-hidden bg-slate-900">
                <video
                  src={introVideoUrl}
                  controls
              ...
              <div className="aspect-video max-w-2xl rounded-2xl border-2 border-dashed border-gray-300 bg-white flex flex-col items-center justify-center gap-3 text-gray-400 cursor-pointer hover:border-indigo-400 hover:text-indigo-500 transition-colors">
                ...
                <div className="space-y-6">
                  ...
                  {courses.length === 0 ? (
                    <div className="py-20 text-center space-y-3">
                      ...
                      <p className="text-gray-500">Hələ heç bir kurs yoxdur.</p>
                      {isOwner && (
                        <Link to="/dashboard/teacher" className="inline-block text-sm text-indigo-600 hover:text-indigo-700">
                ...
                <div className="space-y-4">
                  {competitions.length === 0 ? (
                    <div className="py-20 text-center">
                      ...
                      <p className="text-gray-500">Hələ heç bir yarışma yoxdur.</p>
                    </div>
                    ...
                    <motion.div
                      ...
                      className="bg-white border border-gray-200 rounded-xl p-4 flex items-center justify-between shadow-sm hover:border-indigo-200 transition-colors"
                    >
                      <div>
                        <div className="flex items-center gap-2 flex-wrap">
                          <h3 className="font-semibold text-sm text-gray-900">{comp.title}</h3>
                          <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                            comp.status === 'active' ? 'bg-emerald-50 text-emerald-600 border border-emerald-200' :
                            comp.status === 'upcoming' ? 'bg-indigo-50 text-indigo-600 border border-indigo-200' :
                      ...
                      <Link
                        ...
                        className="text-xs font-semibold text-indigo-600 hover:text-indigo-700 transition-colors shrink-0 ml-4"
                      >
                        Gör →
                      </Link>
                    </motion.div>
              ...
              {activeTab === 'Rəylər' && (
                <div className="space-y-6">
                  ...
                  <div className="flex items-center gap-6 bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
                    <div className="text-center">
                      <p className="text-5xl font-bold text-amber-500">{rating.toFixed(1)}</p>
                      <StarRating value={rating} size={18} />
                      <p className="text-xs text-gray-400 mt-1">{reviewCount} rəy</p>
                    </div>
                  ...
                  {reviews.length === 0 ? (
                    <div className="py-16 text-center">
                      <p className="text-gray-500">Hələ heç bir rəy yoxdur.</p>
                      <p className="mt-1 text-sm text-gray-400">Rəy bölməsi real istifadəçi rəyləri ilə formalaşacaq.</p>
                    </div>
                  ) : (
                    <div className="space-y-4">
              ...
              {activeTab === 'Haqqında' && (
                <div className="max-w-2xl space-y-6">
                  <div>
                    ...
                    <p className="text-gray-600 leading-relaxed">{teacherLongBio}</p>
                  </div>
                  ...
                  {isOwner && (
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <h3 className="font-semibold text-indigo-700">Müəllim CRM Paneli</h3>
                      </div>
                      <div className="grid grid-cols-3 gap-3">
                        {[
                          ...
                          <div key={label} className="bg-white border border-indigo-100 rounded-xl p-3 text-center">
                            <p className="text-lg font-bold text-indigo-700">{value}</p>
                            <p className="text-xs text-indigo-500/80 mt-0.5">{label}</p>
                          </div>
                        ))}
                      </div>
                      <Link
                        to="/dashboard/teacher"
                        className="flex items-center justify-center gap-2 w-full py-2.5 border border-indigo-200 bg-white rounded-xl text-sm text-indigo-600 hover:bg-indigo-50 transition-colors"
                      >
                        Tam CRM panelini aç
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
